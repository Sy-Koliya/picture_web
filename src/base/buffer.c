...
struct buffer_s {
    buf_chain_t *first;
    buf_chain_t *last;
    buf_chain_t **last_with_datap;   //last processed chain
    uint32_t total_len;                     
...
buf_chain_new(uint32_t size) {
    ...
    if ((chain = (buf_chain_t*)malloc(to_alloc)) == NULL)
        return (NULL);
...
buf_chain_free_all(buf_chain_t *chain) {
    //如果总大小超过 4mb 可以丢入线程池进行处理 ，如果丢入线程池处理
    //buf_chain_t total_size > ? thread() swap buff.first buf_chain_t tmp = NULL
    buf_chain_t *next;
...
static int buffer_drain(buffer_t *buf, uint32_t len) {
    ...
    if (len >= old_len) {
        ...
        chain->off -= remaining;
        buf->last_read_pos=buf->last_read_pos-len;
    }
...
/*
search 函数 如果匹配成功 返回指向sep末尾的下标，失败返回0，异常返回-1
当sep长度大于REQURIRLEN_WITH_KMP时 采取KMP匹配

*/


static int buffer_search_kmp(buffer_t *buf, const char *sep, int seplen) {
    // 构造KMP的lps数组（Longest Prefix Suffix，部分匹配表）
    int *lps = (int *)malloc(sizeof(int) * seplen);
    if (!lps)
        return -1; // 分配失败
    ...
    bytes -= buf->last_read_pos; //剩余多少bytes
    int from = chain->off - bytes; //开始点

    int j = 0;    // 当前匹配到 pattern 中的下标

    for (i = buf->last_read_pos; i <= buf->total_len; i++) {
        char c = (char)chain->buffer[chain->misalign + from];
        // 如果匹配不成功，则利用lps数组回溯pattern下标 j
        while (j > 0 && c != sep[j])
            j = lps[j - 1];
        if (c == sep[j])
            j++;
...
int buffer_search(buffer_t *buf, const char* sep, const int seplen) {
    ...
    for (i = buf->last_read_pos; i <= buf->total_len - seplen; i++) {
        if (check_sep(chain, from, sep, seplen)) {  //if the sep is so long can use kmp optimize this process
            buf->last_read_pos = 0;
            return i+seplen;
        }
        ++from;
